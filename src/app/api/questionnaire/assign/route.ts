...
export async function POST(req: NextRequest) {
	try {
		...
		const { questionnaireId, courseId, chapterId } =
...
//   courseId: z.string().min(1, "Course ID is required"),
//   chapterId: z.string().min(1, "Chapter ID is required"),
//   type: z.enum(["quiz", "finalExam"]), // Add type field
// });
...
//     const { questionnaireId, courseId, chapterId, type } = assignSchema.parse(body);
//     console.log("Parsed body:", { questionnaireId, courseId, chapterId, type });
...
//     // 5️⃣ If a chapter is provided, update the chapter's questionnaireId or finalExamId based on type
//     if (chapterId) {
//       await db
//         .update(chapters)
//         .set({
//           [type === "quiz" ? "questionnaireId" : "finalExamId"]: questionnaireId, // Assign the questionnaire
//         })
//         .where(eq(chapters.id, chapterId));
//     }
