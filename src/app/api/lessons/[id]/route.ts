...
export async function GET(
    ...
    try {
        const lessonId = params.id; // Removed await since params.id is synchronous
        ...
        if (token?.sub) {
            ...
            // Ensure enrolledCourses is always treated as an array
            const enrolledCourses = userData[0]?.enrolledCourses 
                ? Array.isArray(userData[0].enrolledCourses) 
