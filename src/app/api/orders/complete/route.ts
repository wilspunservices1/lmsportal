...
import { sendEmail } from "@/libs/email/emailService"; // Assuming your email service is in services folder
...
export async function POST(req: Request) {
	try {
		...
		const emailTemplateData = {
			...
			orderItems: items.map((item) => `${item.name} - $${item.price}`).join(", "),
		};
		...
		return NextResponse.json({ error: "Failed to complete the order." }, { status: 500 });
	}
}
