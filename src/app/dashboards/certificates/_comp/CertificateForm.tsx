	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (certificateImage) {
			try {
				...
				uploadForm.append("file", certificateImage);
				...
				const cloudinaryUrl = uploadResult.imgUrl;
				submitCertificateForm(cloudinaryUrl); // ✅ Pass the Cloudinary URL
			} catch (err) {
		...
		>
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 mb-4 gap-y-4 gap-x-6">
				...
				<div>
					...
					/>
					{formErrors.courseId && (
						<p className="text-xs ml-2 text-red-500 mt-1">{formErrors.courseId}</p>
					)}
				</div>
				...
				<div>
					...
					{formErrors.orientation && (
						<p className="text-xs text-red-500 mt-1">{formErrors.orientation}</p>
					)}
				</div>
				...
				<div>
					...
					{formErrors.maxDownloads && (
						<p className="text-xs text-red-500 mt-1">{formErrors.maxDownloads}</p>
					)}
				</div>
				...
				<div className="xl:col-span-2">
					...
					/>
					{formErrors.description && (
						<p className="text-xs ml-2 text-red-500 mt-1">{formErrors.description}</p>
					)}
				</div>
			</div>
