...
const CourseCard2 = ({ course, card, isList, isNotSidebar, enrolledCourses }) => {
	...
	const lessonId = chapters?.length
		? chapters
				.sort((a, b) => parseInt(a.order) - parseInt(b.order)) // ✅ Sort chapters by order
				.flatMap(
					(chapter) =>
						chapter.lectures?.sort((a, b) => parseInt(a.order) - parseInt(b.order)) ||
						[]
				) // ✅ Sort lectures inside chapters
				?.find((lecture) => lecture.id)?.id // ✅ Get the first lecture ID
		: null;
