...
const LessonAccordion = ({
	...
	const handleQuizStart = async (quiz) => {
		try {
			console.log("Fetching quiz data for questionnaireId:", quiz.questionnaireId);
			const response = await fetch(
			...
			<ul className="accordion-container curriculum">
									...
									{questionnaires[chapter.id] && (
										...
										>
											<div className="flex items-center justify-between">
												...
												<div className="flex items-center">
													{(() => {
														...
														const attempts =
															quizId &&
															typeof quizAttempts[quizId] === "number"
																? quizAttempts[quizId]
																: 0;
											...
											<p className="text-sm text-gray-600">
												{questionnaires[chapter.id].questions.length}{" "}
												Questions
											</p>
											...
											<div className="quiz-score-box bg-pink-500 text-white text-sm px-3 py-1 rounded mt-2">
												Score:{" "}
												{Number(
													quizScores[questionnaires[chapter.id]?.id]
												) || 0}
												%
											</div>
										</div>
				...
				{isEnrolled && (
					<>
						...
						{isEnrolled && totalChapters > 0 && completedChapters === totalChapters && (
							<>
