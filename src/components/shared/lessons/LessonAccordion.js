...
const LessonAccordion = ({
							...
							({ chapterId, questionnaireId }) =>
								...
								fetch(
									...
									.catch((error) => {
										...
										return null; // ✅ Prevents breaking all requests
									})
						);
	...
	const updateScoreInAccordion = (questionnaireId, scorePercentage) => {
		...
		if (!scoreBox) {
			...
			scoreBox.style.background = "#0A616F";
		...
		submitButton.onclick = async () => {
			try {
				...
				if (!Number.isFinite(updatedAttempts)) {
					setQuizAttempts((prev) => ({
						...prev,
						[quiz.questionnaireId]: 1, // Assume max attempts to prevent rendering error
					}));
				}
