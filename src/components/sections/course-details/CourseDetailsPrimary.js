...
import "@/styles/fonts.css"; // Create this file if it doesn't exist
...
const CourseDetailsPrimary = ({
  ...
  const CERTIFICATE_WIDTH = 842; // A4 width in pixels
  const CERTIFICATE_HEIGHT = 595; // A4 height in pixels

  // Updated buildCertificateHTML function with responsive scaling
  function buildCertificateHTML(certificate_data_url, placeholders = []) {
    return `
      <div style="
        position: relative;
          ...
          .map(
            ...
            <div 
              style="
                ...
                left: ${ph.x}px;
                top: ${ph.y}px;
                font-size: ${ph.font_size || 16}px;
                color: ${ph.color || "#000"};
                font-family: ${ph.font_family || "Arial"};
                white-space: nowrap;
  ...
  // Add this helper function to determine font class
  ...
  // Updated openFullScreenCertificate function with responsive scaling
            ...
            (ph) => `
              <div 
                ...
                style="
                  ...
                  font-weight: ${ph.font_weight || "normal"};
                  font-style: ${ph.font_style || "normal"};
                "
              >
                ${ph.value ?? ph.label ?? ""}
              </div>
        ...
        function downloadCertificate() {
          ...
          html2canvas(container, {
            ...
            onclone: function(clonedDoc) {
              ...
              const clonedContainer = clonedDoc.querySelector('.certificate-container');
              clonedContainer.style.width = '${CERTIFICATE_WIDTH}px';
              clonedContainer.style.height = '${CERTIFICATE_HEIGHT}px';
            }
          }).then(canvas => {
  ...
  // Updated handleCertificateSelect function
  const handleCertificateSelect = async () => {
    ...
    try {
      ...
      // Fetch the user details (for student name)
      const userResponse = await fetch(`/api/user/${session.user.id}`);
      if (!userResponse.ok) throw new Error("Failed to fetch user data");
      const userData = await userResponse.json();
      const studentName = userData?.name || "Student Name"; // Fallback

      // Fetch the course details to get the certificateId
      ...
      // Check if the course has a certificateId
      ...
      Swal.fire({
        ...
        html: buildCertificateHTML(certificate_data_url, filledPlaceholders),
        ...
        didOpen: () => {
          // Adjust the popup size to fit the certificate
