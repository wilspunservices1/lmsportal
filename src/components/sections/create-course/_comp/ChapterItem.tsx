...
//   const [selectedQuestionnaire, setSelectedQuestionnaire] =
//     useState<string>("");
//   const [selectedFinalExam, setSelectedFinalExam] = useState<string>("");
//   const [showAssignModal, setShowAssignModal] = useState(false);
//   const [selectedCourse, setSelectedCourse] = useState<string>("");
//   const [selectedChapterId, setSelectedChapterId] = useState<string>("");
...
//   const fetchQuestionnaires = useCallback(async () => {
//     if (isLoadingQuestionnaires || availableQuestionnaires.length > 0) return;
...
//     }
//   }, [showAlert, isLoadingQuestionnaires]);

//   useEffect(() => {
//     let mounted = true;

//     const loadQuestionnaires = async () => {
//       if (mounted && availableQuestionnaires.length === 0) {
...
//   const handleAssignQuestionnaire = async (type: "quiz" | "finalExam") => {
//     const selectedId =
//       type === "quiz" ? selectedQuestionnaire : selectedFinalExam;
...
//       showAlert("error", "Course ID or Chapter ID is missing.");
//       return;
//     }
...
//       if (!response.ok) {
//         throw new Error(data.error || `Failed to assign ${type}`);
//       }

//       // Update the local chapter state with the server response
//       const updatedChapter = {
//         ...chapter,
//         [type === "quiz" ? "questionnaireId" : "finalExamId"]: selectedId,
//       };
...
//       showAlert(
//         "error",
//         error instanceof Error ? error.message : `Failed to assign ${type}`
//       );
//     } finally {
//       setIsLoading(false);
//     }
...
//         {isSaved && (
//           <div className="absolute top-8 right-6 pr-4 flex items-center">
//             <Tooltip content="Click to expand Lessons" position="left">
//               <button
//                 type="button"
//                 onClick={toggleAccordion}
//                 className="text-gray-500 flex items-center justify-center ml-2 transform transition-transform"
...
//             className="flex-1 p-2 border rounded focus:ring-2 focus:ring-primaryColor"
//             disabled={isLoadingQuestionnaires || isLoading}
//           >
//             <option value="">
//               {isLoadingQuestionnaires ? "Loading..." : "Select a Quiz"}
//             </option>
//             {availableQuestionnaires.map((q) => (
//               <option key={q.id} value={q.id}>
//                 {q.title} ({q.questionsCount} questions)
...
//           </select>
//           <button
//             onClick={() => handleAssignQuestionnaire("quiz")}
//             disabled={!selectedQuestionnaire || isLoading}
//             className="px-4 py-2 bg-primaryColor text-white rounded hover:bg-opacity-90 disabled:opacity-50"
//           >
...
//       {/* Final Exam Section */}
//       <div className="mt-4 border-t pt-4">
//         <h4 className="text-lg font-medium mb-2">Final Exam</h4>
//         <div className="flex gap-4">
//           <select
...
//           >
//             <option value="">
//               {isLoadingQuestionnaires ? "Loading..." : "Select a Final Exam"}
//             </option>
//             {availableQuestionnaires.map((q) => (
//               <option key={q.id} value={q.id}>
//                 {q.title} ({q.questionsCount} questions)
...
//           </select>
//           <button
//             onClick={() => handleAssignQuestionnaire("finalExam")}
//             disabled={!selectedFinalExam || isLoading}
//             className="px-4 py-2 bg-primaryColor text-white rounded hover:bg-opacity-90 disabled:opacity-50"
//           >
//             {isLoading ? "Assigning..." : "Assign Final Exam"}
//           </button>
//         </div>
...
//             <p className="text-sm">
//               Assigned Final Exam:{" "}
//               {availableQuestionnaires.find((q) => q.id === chapter.finalExamId)
//                 ?.title || "Loading..."}
//             </p>
//           </div>
//         )}
