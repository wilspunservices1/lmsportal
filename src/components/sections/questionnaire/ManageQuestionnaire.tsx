...
const ManageQuestionnaire: FC<ManageQuestionnaireProps> = ({
	...
	const fetchSingleQuestionnaire = useCallback(async () => {
		...
		try {
			const response = await fetch(
				`/api/courses/chapters/lectures/questionnaires/${questionnaireId}`
			);
			if (!response.ok)
				throw new Error(
					`Failed to fetch questionnaire: ${response.status} - ${response.statusText}`
				);
	...
	const handleDelete = async (id: string) => {
		if (window.confirm("Are you sure you want to delete this questionnaire?")) {
			try {
				const response = await fetch(
					`/api/courses/chapters/lectures/questionnaires/${id}`,
					{
						method: "DELETE",
					}
				);
				...
				showAlert("error", `Failed to delete questionnaire: ${(error as Error).message}`);
			}
		}
	};
	...
	return (
		<div className="p-6">
			...
			{editingQuizId && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-96">
						<EditQuiz quizId={editingQuizId} onClose={() => setEditingQuizId(null)} />
					</div>
				</div>
			)}
		</div>
